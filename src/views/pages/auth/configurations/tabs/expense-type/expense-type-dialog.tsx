...
import { ExpenseType } from "@/database/tables";

export interface ExpenseTypeDialogProps {
  onComplete: (expenseType: ExpenseType) => void;
...
export default function ExpenseTypeDialog(props: ExpenseTypeDialogProps) {
  ...
  const store = async () => {
    try {
      ...
      const response = await axiosClient.post("expense-types", formData);
      if (response.status === 200) {
        toast({
          toastType: "SUCCESS",
          description: response.data.message,
        });
        onComplete(response.data.expenseTypes);
        modelOnRequestHide();
      }
    } catch (error: any) {
  ...
  const update = async () => {
    try {
      ...
      // 2. update
      let formData = new FormData();
      if (expenseType?.id) formData.append("id", expenseType.id);
      formData.append("english", userData.english);
      formData.append("farsi", userData.farsi);
      formData.append("pashto", userData.pashto);
      const response = await axiosClient.post("expense-types", formData);
  ...
  return (
    <Card className="w-fit min-w-[400px] self-center [backdrop-filter:blur(20px)] bg-white/70 dark:!bg-black/40">
      ...
      <CardFooter className="flex justify-between">
        ...
        <PrimaryButton
          disabled={loading}
          onClick={expenseType ? update : store}
